"""
Global pytest configuration and fixtures for SPX CFD testing framework.
"""
...
sys.path.insert(0, str(project_root / "app" / "SP500_CFD_v1"))
...
def mock_market_data():
    ...
    def _generate_data(
        ...
        if frequency == "5M":
            ...
            times = pd.date_range('09:30', '16:00', freq='5T').time
        ...
        if trend == "uptrend":
            trend_component = np.linspace(0, 100, n_periods)
        elif trend == "downtrend":
            trend_component = np.linspace(0, -100, n_periods)
        elif trend == "sideways":
            trend_component = np.sin(np.linspace(0, 4*np.pi, n_periods)) * 20
        else:  # random
            trend_component = np.cumsum(np.random.normal(0, 0.5, n_periods))
        
        # Generate OHLCV data
        close_prices = base_price + trend_component + np.random.normal(0, 2, n_periods)
        
        # Ensure realistic OHLC relationships
        opens = close_prices + np.random.normal(0, 1, n_periods)
        
        highs = np.maximum(opens, close_prices) + np.abs(np.random.normal(0, 1.5, n_periods))
        lows = np.minimum(opens, close_prices) - np.abs(np.random.normal(0, 1.5, n_periods))
...
def mock_retrieve_class():
    ...
    def mock_get_data(symbol, start_date, end_date, frequency="5M"):
        ...
        data = pd.DataFrame({
            'Open': 4500 + np.random.normal(0, 2, n),
            'High': 4505 + np.random.normal(0, 2, n),
            'Low': 4495 + np.random.normal(0, 2, n),
            'Close': 4500 + np.random.normal(0, 2, n),
...
def sample_trades_data():
    ...
    for i in range(10):
        ...
        trade = {
            ...
            'exit_time': exit_time,
            'direction': direction,
            'entry_price': 4500 + np.random.normal(0, 5),
            'exit_price': 4500 + np.random.normal(0, 5),
            'size': 25,
            'gross_pnl': gross_pnl,
            'transaction_cost': 26.0,  # 25 * 1.0 + 1.0
...
def pytest_configure(config):
    ...
    for marker_name, marker_description in [
        ...
        ("ui", "User interface tests for Streamlit components"),
...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        ...
        if "ui" in item.name or "streamlit" in item.name:
            item.add_marker(pytest.mark.ui)
        
        # Add performance marker to performance tests
        if "performance" in item.name or "speed" in item.name:
